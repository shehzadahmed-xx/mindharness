...
\paragraph{Screening.} Five subjects, raw arm only, three seeds each
(exploratory, no lock). Four sat exactly on the always-no floor in every
seed: \texttt{gpt-oss-120b}, \texttt{gpt-oss-20b}, \texttt{qwen3.8-27b}
(Groq) and \texttt{nemotron-3-ultra-free} (Zen), all $0.667$ with zero
variance. One discriminated: \texttt{laguna-s-2.1-free} at $0.861$
(per-seed $0.833/1.000/0.750$). The four-in-five failure rate is itself
informative --- it explains why every previous sham comparison returned a
null, and it means published attribution results are sensitive to a
subject-selection step that is rarely reported.
...
\texttt{b06ce867}) was frozen before this paper was revised.
