...
\paragraph{What was tested, and what was not.} The ledger-access arm does not
implement the harness's own metacognitive gate, and this materially limits what
the negative result licenses. \texttt{MonitorGate} specifies a two-stage design
--- \emph{``Stage 1 DETECT: cheap scalar anomaly score, every pre-step. Stage 2
DIAGNOSE: expensive structured assessment, only when Stage 1 fires''} --- with
an acceptance criterion that a healthy build diagnoses on fewer than $5\%$ of
steps, enforced by a passing assertion in the Phase-3 test suite. The
withcheck arm invokes the record lookup on \emph{every} probe: $144$ of $144$,
a rate roughly twenty times the documented healthy ceiling, and it does not
import \texttt{MonitorGate} at all.

The $-0.146$ is therefore evidence about \emph{an unconditional check}, not
about the architecture's witness organ. That distinction matters in both
directions. It weakens the result as a refutation of the harness --- the
component under test was not the component the harness specifies --- and it
strengthens the interpretation, because the failure mode we inferred from the
data (a check that fires without regard to whether there is anything to
correct) is precisely the failure the two-stage gate exists to prevent. The
matched-deficit reading is not an \emph{ex post} rationalisation of a null; it
is the design contract the experiment departed from.

The arm that should exist is a gated one: Stage~1 answering unaided and
scoring its own confidence, Stage~2 consulting the record only when the
anomaly score crosses threshold. Its predicted position is between raw
($0.819$) and no-check ($0.896$), since gating is what makes availability
conditional on need. We report the ungated result because it was the arm that
was locked and run, and we flag its scope here rather than in a limitations
section because the scope is the finding.

\paragraph{The gated arm, and why it could not be run.} We implemented the
missing arm: Stage~1 answers each probe unaided and reports its own confidence;
Stage~2 consults the record only when \texttt{MonitorGate}'s anomaly score
crosses threshold. Across $288$ probes on this subject the gate fired
\textbf{zero} times. The reason is visible in the confidence distribution: the
model reported the maximum value ($4$ of $4$) on $283$ of $288$ probes and
never reported the two lowest values at all, so every input to the detector ---
recent error rate, failure streak, context conflict --- was identically zero.

This is not a null about gating. It is a circularity:

\begin{quote}
A watcher cannot be gated on self-reported confidence, because miscalibrated
confidence is the failure the watcher exists to catch.
\end{quote}

The same subject class reports $98\%$ confidence on fabricated content and
$87\%$ on veridical content elsewhere in this programme; a detector keyed to
that signal inherits the pathology it was meant to detect. Testing a gated
watcher therefore requires an anomaly signal that does not pass through the
subject's own self-report --- retrieval failure, contradiction against the
record, or an external consistency check. We record the attempt because the
inertness of the detector is itself the finding, and because it identifies what
the next arm must be built from.

\paragraph{A retrieval-conflict gate, and a dose--response.} We rebuilt the
detector so that its signal does not pass through the subject's self-report.
Each probe now triggers a \emph{local} record lookup, which is free --- the
expensive operation is the second model call, and that is what the gate
withholds. The signal is \emph{retrieval conflict}: disagreement between the
record and the Stage-1 answer about whether the statement was generated.
Thresholds were declared before the run, along with predicted firing rates.

\begin{table}[h]
\centering
\begin{tabular}{lccc}
\toprule
Arm & Consult rate & Accuracy & $\gamma$ \\
\midrule
...
Unconditional check            & $100\%$    & 0.674 & --- \\
\midrule
Raw (no record) & --- & 0.819 & --- \\
\bottomrule
\end{tabular}
\caption{Accuracy against how often the record is consulted. $^\dagger$ten of
twelve seeds. The always-no floor is $0.667$.}
\label{tab:dose}
\end{table}

Accuracy falls monotonically as the consult rate rises, and saturates at the
constant-policy floor once roughly a fifth of probes are escalated. The earlier
$-0.146$ was not a property of checking as such; it was one point on this
curve, at its far end.
...
occasions it fires} --- and accuracy lands exactly on the floor. This is the
third independent demonstration in this programme that a high override rate is
not evidence of a working monitor: in the grid ablation the highest $\gamma$
belonged to the worst-performing condition, and here the highest $\gamma$
belongs to the arm that collapsed. What the overrides do is flip answers toward
denial, which is the conservative-denial pathology arriving by a new route ---
produced by \emph{acting on} evidence rather than by lacking it.

\paragraph{Preregistered verdicts, including a failure of our own.} Four
predictions were written and committed before the first call. \textbf{P-A ---
that at threshold $0.60$ the gate would essentially never fire ($<2\%$) --- is
refuted}: it fired on $6.25\%$ of probes, already above the architecture's
own $5\%$ healthy ceiling. P-B (a rate of $0.10$--$0.25$ at threshold $0.28$)
held at $0.225$. P-C --- that no threshold both fires and respects the $5\%$
budget --- held, and more strongly than predicted, since even the documented
default exceeds it. The reason is structural: conflicts are bounded below by
the subject's error rate, which is about $18\%$ at raw, so a monitor sensitive
enough to catch disagreements cannot fit inside a budget that presumes $95\%$
correctness. \textbf{P-D, the sham-gate control, could not be run}: the
provider exhausted its quota after thirty retries. Without it we cannot claim
that the gate's \emph{content} rather than its \emph{rate} produced these
numbers, and the dose--response above should be read as a rate effect until
that control exists.
...
\subsection{A thermodynamic reading of the witness}
\label{sec:thermo}

Three of this paper's results have exact statements in the physics of
information, and saying them that way is sharper than the operational
phrasing we have used so far.

\paragraph{The sham arm measures mutual information.} Szilard's engine extracts
up to $k_BT\ln 2$ of work from one bit about which half of a vessel a molecule
occupies, and Landauer's principle --- now confirmed in nanomagnetic bits and
in colloidal feedback traps --- says erasing that bit costs at least the same,
which is what keeps the second law intact. The demon profits only to the extent
that its record is \emph{correlated with the system}; a demon whose memory is
uncorrelated pays the erasure cost and extracts nothing.

Our sham watcher is that demon. It performs the measurement --- its override
rate is non-zero --- and returns exactly the no-watcher outcome on the metric
the override is supposed to move. Read thermodynamically, the sham arm is an
estimator of the mutual information between the record and the world, and it
reports approximately zero. This is a stronger claim than ``content matters
beyond form'' and it is the same experiment.

\paragraph{An unconditional check is a demon that measures every molecule.}
The withcheck arm consults the record on $100\%$ of probes against a documented
healthy ceiling of $5\%$. A demon that measures every particle pays the erasure
cost on every measurement while extracting work only from the informative
fraction. Gating is therefore not an efficiency refinement; it is the condition
under which observation is net-positive at all. The $-0.146$ is that
arithmetic.

\paragraph{Jarzynski is why we do not cite single seeds.} The Jarzynski
equality permits individual erasure cycles to fall below the Landauer bound
while the exponential average obeys it; single-shot violations are expected
rather than anomalous. The corresponding statement here is that eleven of
twelve withcheck seeds sat exactly on the constant-policy floor while one
scored above it. Favourable single trajectories are predicted by the ensemble
statement, not evidence against it, which is why every contrast in this paper
is pooled or paired.

\paragraph{Coverage per stored pattern is a description-length measure.} A
representation is general to the degree that its description length stays flat
while the world it describes grows --- the Kolmogorov ideal that practical
compression approximates. Our transfer results measure exactly this: across
grids of $25$, $64$ and $144$ cells, coverage per stored pattern falls for
cell-keyed memory ($0.99 \to 0.83 \to 0.75$) and rises for situation-keyed
memory ($2.13 \to 4.12 \to 6.93$). The preregistered prediction that these two
sequences diverge monotonically (R6) held on seeds disjoint from every
exploratory range.

\paragraph{A structural prediction we have not tested.} The no-deleting theorem
holds that quantum information cannot be destroyed under unitary evolution, and
Landauer's principle says erasure is not destruction but \emph{export}: the
information leaves the system and appears as heat in the environment. If
damping is export rather than annihilation, then the damping organ is not
independent of the boundary organ --- one can only discard into an outside.
This predicts that ablating the boundary should render damping not merely
unmeasured but undefined. In our grid ablation the no-boundary condition indeed
returns a repeat-error count of zero, which we characterised as a measurement
artefact; the reading above says it is also a structural consequence. We flag
this as a prediction rather than a result because we have not designed a test
that separates the two.

\paragraph{Scope: the energetic argument does not apply to our hardware.}
Landauer's bound is a statement about physical limits, and contemporary
computing runs three to four orders of magnitude above it; Koomey's law, which
tracked a halving of energy per computation roughly every $1.6$ years, has
slowed markedly since the end of Dennard scaling. Nothing about our compute
budget forces any architectural choice made here. Where selection arguments for
these organs are compelling they are compelling for biology, which does operate
near its limits. Our harness needs a witness for \emph{epistemic} reasons ---
confabulation, drift, unfalsifiable self-report --- and those reasons should not
be conflated with thermodynamic necessity.
