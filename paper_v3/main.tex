...
discrimination ($1.000$) while every harnessed arm --- with and without
ledger query access --- converged to the same wholesale-denial policy
($0.667$, the always-no baseline). Ledger access did not recover the gap
on this subject: the harness cost $0.333$ of attribution accuracy and the
witness did not buy it back. A role-specialized composite passed both
...
\texttt{gpt-oss-120b} ignores ledger content entirely: when a subject
...
in \texttt{paper\_v2/} (empirical, 21pp, tagged \texttt{paper-v2-final})
