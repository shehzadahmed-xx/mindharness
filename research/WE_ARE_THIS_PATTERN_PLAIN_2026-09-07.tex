...
Table~2 shows each condition three ways: the plain job, what it looks like in a person, and what a bacterium does. The point is direct: smaller flow, fewer materials borrowed, same five conditions. Table~2 is a plain restatement of Table~1 with the person and bacterium columns added.
...
All figures below are working-threshold summaries from our batteries (see Methods and Table~1 for units). Intervals are 95\% ranges where counts exist; the deposit staging manifest (19 locks with SHAs) is in \texttt{research/DEPOSIT\_STAGING\_2026-09-07/} pending minting of a public DOI.
...
\end{axis}
\end{tikzpicture}
\caption{Replication across wirings: the gap between strong and weak models (standardized gap 3.76) dominates the gap between wiring types (0.10), about 37 times larger. Standardized gap means mean difference divided by spread. Failure-mode agreement is plotted in Fig.~4.}
\end{figure}
