...
What it gains: seven million tiny chips could one day hold the full wiring of a human brain at the barrier where energy to hold it equals energy to erase it, where we now run a fly in a browser (Berg et al.\ 2026) [35]; fast matching with big matrices that can compare many things in parallel, once sequential for a human, now parallel as a whole table in one go; local \emph{better} when sparser.
...
\paragraph{Artifact control.} A written record was opened before each test and the storyteller could not edit it (coverage $\ge 95$\%). Claims were scored kept vs removed vs shuffled. Falsification rule: if shuffled is not worse than kept by at least three percent transferred and one percent kept, or the gap is not positive when checked, the claim fails.
\paragraph{Measures.} Leak rate (false claims), drift in twelve turns, unread count, drawdown and lock-in, and changed-over-checked with sham-controlled loss (sham $<$ real required). Human results are reported as descriptive synthesis of cited trials, not new human experiments by the authors.
...
\begin{table}[h]
\centering
\caption{Five jobs: what each does and its named death when blank.}
\begin{tabular}{lll}
\toprule
...
\item A.~Nayebi, What capable agents must know: Selection theorems for robust decision-making under uncertainty, UAI 2026, PMLR v337, 4773--4795. arXiv:2603.02491. \url{https://arxiv.org/abs/2603.02491}
\item J.~Zhang et al., Introspection threshold, arXiv:2607.04277 (2026); T.~Lin et al., AHE, arXiv:2604.25850 (2026); M.~Macar et al., arXiv:2603.21396 (2026); A.~Panickssery et al., NeurIPS (2024); N.~Sofroniew et al., arXiv:2604.07729 (2026).
...
\item S.~Berg et al., Sexual dimorphism in the complete Drosophila male CNS connectome, \textit{Cell} 189, 5504--5526 (2026). \url{https://www.cell.com/cell/fulltext/S0092-8674(26)00942-6} Dataset: \url{https://male-cns.janelia.org/}
\end{enumerate}

\section*{Acknowledgments}
We thank the many readers of the seven shores---from tiny-swimmer studies to market ledgers to small artificial minds---who let us test five jobs at many sizes, and the clear record outside the storyteller that lets us be wrong quickly.

\vspace{1em}
\noindent\textit{Generated 2026-09-07---plain paragraphs from one falsifiable idea: five surfing twenty in middle where milk swirls, made of maintenance, checkable if witnessed, one small step at a time. So the next reader from tiny swimmer to big brain gets: bacterium is you, just smaller loan---without re-deriving everything. For the technical version with symbols, see the companion dense paragraph. Full literature map in LITERATURE\_REVIEW\_2026-09-07.md (disk first, then web 2026-09-08).}
