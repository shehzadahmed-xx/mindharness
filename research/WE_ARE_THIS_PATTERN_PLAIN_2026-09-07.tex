...
\centering
\begin{tikzpicture}
\begin{axis}[ybar, ymin=0, ymax=110, width=0.92\textwidth, height=5.2cm, bar width=22pt, ylabel={Share in its own units (\%)}, symbolic x coords={Skin invent,Two-mem drift,Spotlight unread,Brake drawdown,Watcher theater}, xtick=data, nodes near coords, nodes near coords align={vertical}, xticklabel style={font=\small}]
\addplot coordinates {(Skin invent,98) (Two-mem drift,30) (Spotlight unread,100) (Brake drawdown,43) (Watcher theater,66.7)};
\end{axis}
\end{tikzpicture}
\caption{Named death when each job is blank. Each bar has its own unit, not one shared scale: Skin 98 of 100 claims invented; Two-mem more than 30 of 100 items drift in twelve turns; Spotlight tens of thousands unread; Brake drawdown from 43 to 15; Watcher theater at two of three. Working thresholds; per-measure counts in Methods.}
\end{figure}

\begin{figure}[h]
\centering
\begin{tikzpicture}
\begin{axis}[ybar, ymin=0, ymax=1, width=0.7\textwidth, height=5cm, bar width=30pt, ylabel={Share correct}, symbolic x coords={mixed record,kept record,full loop}, xtick=data, nodes near coords, xticklabel style={font=\small}]
\addplot coordinates {(mixed record,0.667) (kept record,0.674) (full loop,0.903)};
\end{axis}
\end{tikzpicture}
\caption{Record present beats record mixed: mixed record (same size, content broken) two thirds correct and kept record (present) just above it, both below the full loop at 0.903. The rule requires present above mixed by at least three percent on new tasks and one percent on the same task with a positive checked gap.}
\end{figure}

\begin{figure}[h]
\centering
\begin{tikzpicture}
\begin{axis}[width=0.6\textwidth, height=4.6cm, xmin=0, xmax=1, ymin=0.40, ymax=0.55, xtick=\empty, ylabel={Share above zero}, xlabel={Scale check (one count)}, title={964 of 2000 = 0.482, 95\% range [0.460, 0.504]}]
\addplot+[only marks, mark=*, error bars/.cd, y dir=both, y explicit] coordinates {(0.5,0.482) +- (0.0218,0.0219)};
\end{axis}
\end{tikzpicture}
\caption{Scale check: 964 of 2000 items (48 of 100) score above zero when the outside record is present, with a 95\% range of 46 to 50 in 100. Seven of seven present-above-mixed confirmations (range about 65 to 100 in 100). Share above zero means items scoring above zero divided by all items.}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Replication across wirings: the gap between strong and weak models (standardized gap 3.76) dominates the gap between wiring types (0.10), about 37 times larger. Standardized gap means mean difference divided by spread. Failure-mode agreement 32 of 40 (80 of 100, range 65 to 90) and 30 of 40 in the second language (75 of 100, range 60 to 86).}
\end{figure}
