...
\paragraph{Artifact control.} A written record was opened before each test and the storyteller could not edit it. Coverage means claims with a record pointer divided by all claims; target $\ge 95$\% scored by two assignment-derived passes (arm label vs manifest purpose field, blind to outcomes): 18/18 agree, kappa=1.000 (Cohen 1960) [37], degenerate by construction since both passes read run assignment, so this is an assignment check, not a true claim-level reliability estimate; claim-level double-coding with an independent rater is pending (worksheet staged). Claims were scored kept vs removed vs shuffled. Falsification rule (canonical): kept must beat shuffled by at least three percent transferred and at least one percent kept, with a positive gap when checked (sham $<$ real required). Thresholds are working priors from our batteries, not calibrated findings.
