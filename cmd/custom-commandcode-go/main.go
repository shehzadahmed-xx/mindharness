...
const (
	providerKey = "commandcode-go"
	fOpenAI = sdktr.Format("openai.chat")
	fMyProv = sdktr.Format("commandcode-go.chat")
)
	...
	hooks := cliproxy.Hooks{
		OnAfterStart: func(s *cliproxy.Service) {
			models := []*cliproxy.ModelInfo{
				{ID: "laguna-s-2.1-free", Object: "model", Type: providerKey, DisplayName: "Laguna S 2.1 (free while capacity lasts) — GO $1"},
				{ID: "minimax-m3", Object: "model", Type: providerKey, DisplayName: "MiniMax M3 (2× on GO)"},
				{ID: "mimo-v2.5", Object: "model", Type: providerKey, DisplayName: "MiMo V2.5 (99% off)"},
			}
			for _, a := range core.List() {
				if strings.EqualFold(a.Provider, providerKey) || strings.Contains(strings.ToLower(a.Provider), "command") {
	...
	svc, err := cliproxy.NewBuilder().
		...
		WithServerOptions(
			...
			api.WithRequestLoggerFactory(func(cfg *config.Config, cfgPath string) logging.RequestLogger {
				...
				return logging.NewFileRequestLoggerWithOptions(true, "logs", dir, cfg.ErrorLogsMaxFiles)
			}),
		).
		WithHooks(hooks).
		Build()
