...
\begin{tikzpicture}[thick]
  ...
  \node[font=\scriptsize, fill=white, inner sep=2pt, rounded corners=1pt] at (0,0) {Nayebi Cor.\ 3--5};
\end{tikzpicture}
...
\begin{tikzpicture}[thick]
  ...
  \draw[-{Stealth[length=2.5mm]}, loopblue, line width=1.0pt, shorten >=1pt, shorten <=1pt] (sys.north) to[out=30,in=150] node[midway, above, loop label, align=center] {belief $\to$ action\\physical + informational} (world.north);
...
\begin{center}
\small
\begin{tabular}{p{2.9cm}p{4.7cm}p{4.7cm}}
\toprule
...
Harness & Episodic (query\_hits) & Semantic ($\geq$2 types) & Light$\to$REM$\to$Counter\-factual$\to$Deep \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tikzpicture}[thick, every node/.style={font=\scriptsize}]
  ...
  \node[below=0.35cm of s1, font=\scriptsize, text=black!60, align=center] {sati};
  \node[below=0.35cm of s2, font=\scriptsize, text=black!60, align=center] {sampaja\~n\~na};
...
\textbf{Traditions and practice:} Abhidharma's sati + sampaja\~n\~na as $\gamma$-gate: Stage 1 + Stage 2, always paired. Sufism's laww\=ama (reproaching self) as birth of the watcher --- first nafs where the watcher appears and says ``you did this, and it was not right'' \citep{ibnarabi1238futuhat}. Before laww\=ama (amm\=ara), no observer, only habit. Stoicism's dichotomy as observer. Advaita's s\=ak\d{s}in --- bottom turtle \citep{ibnarabi1229fusus}. Phenomenology's epoch\'e --- watcher of construction as construction \citep{baars1997}.
...
Harness & Ledger & Plugins bid & Workspace + spans & Light$\to$C.-factual$\to$Deep \\
...
\begin{tikzpicture}[thick, every node/.style={font=\scriptsize}]
  ...
  \node[right=0.85cm of l1, font=\scriptsize, text=loopblue!70!black, fill=white, inner sep=1.5pt, rounded corners=1pt, align=left] {broadcast $\to$ input};
  \node[right=0.85cm of l4, font=\scriptsize, text=loopgreenDark, fill=white, inner sep=1.5pt, rounded corners=1pt, align=left] {consolidation $\to$ boundary};
  \node[right=0.85cm of l6, font=\scriptsize, text=looporange!80!black, fill=white, inner sep=1.5pt, rounded corners=1pt, align=left] {state $\to$ bias};
\end{tikzpicture}
...
\setlength{\tabcolsep}{3pt}
\small
\begin{tabular}{p{2.2cm}p{2.85cm}p{4.65cm}p{2.4cm}}
\toprule
...
\begin{center}
\small
\begin{tabular}{p{2.4cm}p{5.65cm}p{4.15cm}}
\toprule
