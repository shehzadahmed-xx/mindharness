...
\begin{abstract}
Any system that must act competently under uncertainty, over time, in a world that contains itself, faces the same constraint regardless of substrate. We formalize this constraint as the \emph{dynamic reflexive loop} --- a system whose actions rewrite the conditions for its next actions because the world it acts on contains itself --- and prove, via selection theorems \citep{nayebi2026}, that any stable pattern in such a world must develop five organs or be exploited by the task distribution that needs them: \textbf{boundary}, \textbf{two-speed memory}, \textbf{salience gate}, \textbf{damping}, and \textbf{internal observer} (our executable interpretation of Nayebi Cor.~3--5). The same five, on our reading, appear at every scale we can measure --- from NMDA coincidence detection at the synapse to global workspace competition in the mind to funding invariants in the market --- and convergent insights were independently found by contemplative traditions that watched a reflexive system long enough (Abhidharma's 52 factors, Sufism's stations, Advaita's veil, and others, detailed in \S\ref{sec:traditions}). We implement all five as an executable cognitive harness (11 modules, 8 layers, 78/78 tests green) with an audit trail that converts convergence from coincidence into evidence, and show where the framework points when extended to its limit: the universe as the largest loop.
\end{abstract}
...
Internal observer & Slower loop watching faster one, veto-able; $\gamma$ = changed/diagnosed (Stage 1 sati, always-on anomaly, cheap; Stage 2 sampaja\~n\~na, triggered diagnosis, expensive). \emph{Example:} ledger check before the next claim; compliance guard $\gamma=0$ proves inert watching. \\
...
Light$\to$Deep & Consolidation pipeline: Light (dedupe) $\to$ REM (concept extract) $\to$ Counterfactual (variant$\to$simulate$\to$regret) $\to$ Deep (promote if $\geq$2 query types) \citep{tononi2014}. \emph{Example:} REM ablation is dream-deprivation. \\
...
\textit{Internal observer:} MonitorGate Stage 1 scores anomaly; if triggered, Stage 2 diagnoses $\to$ trust/retry/revise/abstain, $\gamma$ = changed/diagnosed. Without the observer, beautiful reports, zero steering.
...
\caption{One turn through the five organs --- concrete walkthrough for the new reader. Every forward reference in the paper (ledger, $\gamma$, Light$\to$Deep, AffectState) is anchored here before its formal definition. Occasionalism (\S\ref{sec:occasionalism}) and the full tradition maps (\S\ref{sec:traditions}) can be read as reference and deferred without breaking this flow.}
\label{fig:oneturn}
\end{figure}
...
Ahmed, S., et al. (2026). Witnessed Introspection in Assembled Language-Agent Systems (v2, 19pp, 10 figures, preregistered SHA-256 locks). MindHarness, tagged \texttt{paper-v2-final}, \texttt{github.com/shehzadahmed-xx/mindharness}.

\bibitem[Paper v3 2026]{paper-v3}
Ahmed, S., et al. (2026). The Dynamic Reflexive Harness: Five Invariants from Molecules to Markets to Machines (v3, $\sim$35pp, App.\ A Cordis + App.\ B Mirror). MindHarness, \texttt{github.com/shehzadahmed-xx/mindharness}.
