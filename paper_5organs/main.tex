...
\midrule
Boundary & Remove cleft $\to$ no Hebb & Lesion perirhinal $\to$ no recognition & Remove ledger $\to$ 98\% on fabrications \\
Two-speed & Block late LTP $\to$ no consolidation & Lesion hippocampus $\to$ no one-shot & Ablate Light/REM/Deep: each carries work \\
Salience & Block DA/NE $\to$ no tagging & Blunt affect $\to$ no distinction & Remove AffectState $\to$ saturation \\
Damping & Block LTD/BCM $\to$ seizure & Block SHY $\to$ saturation & Remove cull\_failed $\to$ hallucination loop \\
Observer & Block efference copy $\to$ can't tickle & Lesion PFC $\to$ no veto & Set $\gamma=0$ (compliance guard) $\to$ beautiful reports, zero steering \\
\bottomrule
\end{tabular}
\end{center}
