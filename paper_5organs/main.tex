...
\textbf{Harness:} \texttt{MonitorGate}: \texttt{DetectSignals} (error rate, failure streak, context conflict, embodied strain) $\to$ \texttt{evaluate()} $\to$ \texttt{anomaly\_score} $\to$ if triggered, \texttt{diagnose()} $\to$ \texttt{policy} (trust/retry/revise/abstain) $\to$ \texttt{override\_rate()} ($\gamma$). Compliance guard as permanent negative control ($\gamma=0$ by construction). Every turn, cheap; only when triggered, expensive. The observer is not a module. It is the \emph{gating} that determines whether the next broadcast will be the winner's content or the veto's abstention \citep{macar2026,cao2026}.
