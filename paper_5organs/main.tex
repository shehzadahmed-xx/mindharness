...
\author{Shehzad Ahmed \\ \small Independent University, Bangladesh \\ \small \texttt{github.com/shehzadahmed-xx/mindharness}}
\date{August 27, 2026}
...
\textit{Molecule (LTD/BCM/Oja):} Hebb without damping is seizure --- fire together $\to$ wire together $\to$ fire more $\to$ everything fires \citep{tononi2014}. LTD pulls AMPA out; BCM $\theta_m$ slides (quiet more plastic, hyperactive less) \citep{bienenstock1982}; Oja normalizes ($\Delta w \propto y(x - yw)$ gives PCA, prevents winner-take-all) \citep{oja1982}. Loop: potentiation $\to$ threshold slides $\to$ next potentiation harder $\to$ potentiation self-limits. Without it, one synapse takes all strength. Each mechanism is its own paper: LTD as active forgetting, BCM as metaplasticity, Oja as normalization.
...
\textit{Molecule (BCM metaplasticity):} Quiet neurons lower $\theta_m$ (more plastic), hyperactive raise it (less) \citep{bienenstock1982}. Loop: activity $\to$ threshold slides $\to$ next activity's plasticity different $\to$ next threshold slides differently. The synapse watches its own recent activity and vetoes its own next plasticity. Each mechanism is its own paper: BCM as metaplasticity, Oja as normalization, STDP as causal veto.
...
\textit{Universe (awareness):} Measurement perturbs what it measures $\to$ next measurement must be about perturbed world $\to$ next theory must describe new world. Loop: observe $\to$ world now contains observation $\to$ next observation different. The observer at universe scale is the practice that sustains the loop being what it is at each instant.
...
\textit{Boundary:} Narrative self (L6) --- ``this story is mine'' vs ``that story was handed to me'' --- CAS-revisioned (N $\to$ N+1 or rejected), cause-signed (action-outcome vs narrative-summary vs external-write, so narrated traits cannot file as lived ones), verbatim-archived, diffable, re-injected every turn or drifts $>$30\% in 12 turns \citep{choi2024}. The self is not stored. It is performed.
...
\textit{Loop, fully:} Measurement (any interaction that makes a property definite) $\to$ which description wins? Theory vs theory, instrument vs instrument --- salience (precision, funding, attention) $\times$ utility (predictive power) $\to$ law (winning description made globally available as ``how the universe works'' --- becomes the reality every next measurement must be about) $\to$ paradigm (slow structural memory that encodes which competitions tended to win) $\to$ state update: the universe as described is now the universe that the next description must describe, now perturbed by the last description's consequences $\to$ every theory, once believed and acted on, changes the world that the next theory must describe --- reflexive loop where the map becomes part of the territory. Not mysticism. Same loop, at the scale where the system and the world are no longer separable even in principle. Each mechanism here is its own paper: measurement as boundary, theory competition as salience, law as broadcast, paradigm as consolidation --- four papers, one loop at the scale of everything.
...
Ahmed, S., et al. (2026). Witnessed Introspection in Assembled Language-Agent Systems (v2, 21pp, 10 figures, preregistered SHA-256 locks). MindHarness, tagged \texttt{paper-v2-final}, \texttt{github.com/shehzadahmed-xx/mindharness}.
...
Ahmed, S., et al. (2026). The Dynamic Reflexive Harness: Five Invariants from Molecules to Markets to Machines (v3, 29pp, App.\ A Cordis + App.\ B Mirror). MindHarness, \texttt{github.com/shehzadahmed-xx/mindharness}.

\bibitem[Merleau-Ponty 1945]{merleauponty1945}
Merleau-Ponty, M. (1945). \emph{Ph\'enom\'enologie de la perception}. Gallimard.

\bibitem[Husserl 1913]{husserl1913}
Husserl, E. (1913). \emph{Ideen zu einer reinen Ph\"anomenologie und
ph\"anomenologischen Philosophie I}. Niemeyer.
