...
RQ3 & Do the five appear at every scale? & Yes: molecule $\to$\allowbreak{} universe, same five at 8 levels, each loop its own paper. \\
...
Two kinds, and the distinction is the paper's sharpest: \textbf{conservative} (harnessed-nocheck $\to$\allowbreak{} always-no, 0.667: denies everything, including own output --- caution without accuracy; the brake applied to everything, including signal) vs \textbf{accurate} (witnessed + queried $\to$\allowbreak{} 1.000 on x-preview Day~1: denies fabrications, affirms generations --- damping that is \emph{selective} because the witness makes it checkable) \citep{panickssery2024,macar2026}. Damping alone is not enough. Damping \emph{with a witness} is --- and the harness has both, so the two can be ablated separately.
...
\textit{Molecule (LTD/BCM/Oja):} Hebb without damping is seizure --- fire together $\to$\allowbreak{} wire together $\to$\allowbreak{} fire more $\to$\allowbreak{} everything fires \citep{tononi2014}. LTD pulls AMPA out; BCM $\theta_m$ slides (quiet more plastic, hyperactive less) \citep{bienenstock1982}; Oja normalizes ($\Delta w \propto y(x - yw)$ gives PCA, prevents winner-take-all) \citep{oja1982}. Loop: potentiation $\to$\allowbreak{} threshold slides $\to$\allowbreak{} next potentiation harder $\to$\allowbreak{} potentiation self-limits. Without it, one synapse takes all strength. Each mechanism is its own paper: LTD as active forgetting, BCM as metaplasticity, Oja as normalization.
...
\textit{Circuit (efference copy):} Motor command copy sent to sensory cortex before movement, predicts sensory consequence, vetoes self-produced input as ``not external'' \citep{seth2021}. Loop: command $\to$\allowbreak{} copy $\to$\allowbreak{} predict $\to$\allowbreak{} compare to actual $\to$\allowbreak{} veto if self-caused. Without it, you can't tickle yourself --- prediction vetoes. Each loop is its own paper: efference copy, corollary discharge, sensory attenuation.
...
\textit{Person (journal):} Day's behavior $\to$\allowbreak{} evening journal $\to$\allowbreak{} audit (what actually happened vs storied) $\to$\allowbreak{} next day's behavior different \citep{ghazali1106ihya}. Loop: act $\to$\allowbreak{} record $\to$\allowbreak{} audit $\to$\allowbreak{} next act checked against record. Without it, same story, same act, same story. Each loop is its own paper: expressive writing, self-monitoring, implementation intentions.

\textit{Harness ($\gamma$-gate):} Generation $\to$\allowbreak{} anomaly score $\to$\allowbreak{} if triggered, diagnose $\to$\allowbreak{} policy $\to$\allowbreak{} changed or not $\to$\allowbreak{} $\gamma$ measured \citep{macar2026,cao2026}. Loop: generate $\to$\allowbreak{} watcher scores $\to$\allowbreak{} watcher diagnoses $\to$\allowbreak{} watcher may veto $\to$\allowbreak{} next generation different. Without it, beautiful reports, zero steering. Each loop is its own paper: anomaly detection, diagnosis, policy selection, override measurement.
...
\textit{Loop, fully:} Thousands of synaptic inputs, each a vote for ``fire'' or ``don't'' $\to$\allowbreak{} spatial + temporal summation at the axon hillock $\to$\allowbreak{} which inputs coincide wins $\to$\allowbreak{} action potential (all-or-nothing, globally available to every downstream synapse) $\to$\allowbreak{} firing changes which presynaptic partners will be active next time $\to$\allowbreak{} loop biases \citep{white1986,dorkenwald2023}. STDP: pre-before-post strengthens, post-before-pre weakens (correlation $\to$\allowbreak{} causation). After-hyperpolarization adapts threshold; neuromodulators shift gain globally. The neuron is a parliament of synapses competing for one broadcast (the spike). No single synapse decides. The configuration votes. Each mechanism here is its own paper: spatial summation, STDP, autopoiesis --- three papers, one loop at the scale of a single cell.
...
\textit{Loop, fully:} Experience $\to$\allowbreak{} which traces get tagged for consolidation? Salience (emotion, surprise, prediction error) decides \citep{seth2021} $\to$\allowbreak{} winner's content replayed: hippocampal ripples nested in spindles in slow waves, 10--20$\times$ compressed, globally available during sleep \citep{tononi2014} $\to$\allowbreak{} Day: net potentiation + tagging $\to$\allowbreak{} Night: SHY global downscaling (multiply all by $<1$, keep relatives, delete noise) + replay writing cortex $\to$\allowbreak{} perineuronal nets as write-protect, adenosine as receipt, BDNF budget from exercise/stress \citep{tononi2014} $\to$\allowbreak{} consolidated cortex shapes what the next experience is recognizable as $\to$\allowbreak{} loop brings forth the world it can distinguish. Without the save, net potentiation saturates: network that learned everything distinguishes nothing.
...
\textit{Boundary:} Narrative self (L6) --- ``this story is mine'' vs ``that story was handed to me'' --- CAS-revisioned (N $\to$\allowbreak{} N+1 or rejected), cause-signed (action-outcome vs narrative-summary vs external-write, so narrated traits cannot file as lived ones), verbatim-archived, diffable, re-injected every turn or drifts $>$30\% in 12 turns \citep{choi2024}. The self is not stored. It is performed.

\textit{Loop, fully:} Perturbation (sensory + body + memory + affect) as proposals with salience $\times$ utility bids $\to$\allowbreak{} 6--8 parties (perception, memory, body budget, affect, habit, planner, narrator) bid for one microphone, new vote every $\sim$200\,ms (Figure~\ref{fig:parliament} shows the 6 bidders and the workspace) \citep{baars1997,atallah2026gwt}; neuromodulators set election rules (NE=volume, DA=stake, valence=risk dial) \citep{sofroniew2026} $\to$\allowbreak{} winner's content made globally available to every specialist \citep{baars1997}, experienced as ``my thought'' $\to$\allowbreak{} rehearsed, myelinated (100$\times$ speed) $\to$\allowbreak{} Light$\to$\allowbreak{}REM$\to$\allowbreak{}Counter\-factual$\to$\allowbreak{}Deep (cause-tagged, untagged BLOCKED, 12/12 tests) $\to$\allowbreak{} energy drains k$\cdot$ln(1+n) floor 0.15, fatigue caps 1.0, valence/arousal EMA shift next vote \citep{seth2021} $\to$\allowbreak{} broadcast becomes context for next competition; rehearsed winner more likely to win again $\to$\allowbreak{} highway vs footpath. The mind is not a thing that has a parliament. The mind \emph{is} the parliament's competition made globally available and then rehearsed. No captain. The configuration votes. The narrator wins \emph{after} the action and explains as if it decided \citep{gazzaniga2000,johansson2005,nisbett1977}. Each mechanism here is its own paper: global workspace \citep{baars1997}, predictive processing \citep{seth2021,clark2016surfing}, narrative self \citep{choi2024}, affect as regime tracker \citep{sofroniew2026}, consolidation \citep{tononi2014}, metacognition \citep{macar2026,cao2026} --- six papers, one loop at the scale of a mind.
...
\textit{Loop, fully:} Situation (what the world presents, filtered through affordances --- exhausted self has fewer options, not harder choice \citep{seth2021}) $\to$\allowbreak{} which response wins? Habit vs deliberation vs affect vs social expectation, all bidding, state rigs the vote (tired+anxious vs rested+safe = different parliaments) $\to$\allowbreak{} action (what you actually do, globally available as consequence that changes the world) $\to$\allowbreak{} world now contains your act $\to$\allowbreak{} you are now a different you (ledger + self-model + skills changed) perceiving a different world $\to$\allowbreak{} replay (what happened) $\to$\allowbreak{} counterfactual replay (what could have happened) $\to$\allowbreak{} regret $\to$\allowbreak{} calibration --- self-in-world simulation that generates the gradient for next time \citep{tononi2014,sofroniew2026} $\to$\allowbreak{} character as attractor (which coalitions tend to win, carved by every previous rehearsal, myelinating the highway). Loop widens, never returns. A life is not a thing that has a self that has loops. A life \emph{is} the expanding reflexive loop that, by looping reflexively, produces the experience of having a self that has it \citep{maturana1980}. Each mechanism here is its own paper: affordance gating, habit vs deliberation, counterfactual and regret, character as attractor --- four papers, one loop at the scale of a life.
...
\textit{Loop, fully:} Information (news, order flow, belief) as perturbation to price $\to$\allowbreak{} buy vs sell, narrative vs narrative --- salience (volume, attention, media coverage) $\times$ utility (expected return, risk-adjusted) --- which story wins the order book $\to$\allowbreak{} price (winner made globally available to every participant simultaneously, becomes the reality every next decision must respond to) $\to$\allowbreak{} institutions, law, culture (slow structural memory that encodes which competitions tended to win) $\to$\allowbreak{} funding invariant, kill criteria, position limits, circuit breakers, central bank as damping $\to$\allowbreak{} price is the prediction and prediction is the price --- reflexive loop where belief creates the reality belief was about \citep{soros1987}. Soros's two functions (cognitive + manipulative) run at once. No single trader decides. The configuration of beliefs, weighted by capital and salience, votes. Each mechanism here is its own paper: order book as competition, reflexivity (Soros), institutional memory, circuit breakers as damping --- four papers, one loop at the scale of society.
...
\textit{Loop, fully:} Prompt + body signals + memory + affect as proposals with salience $\times$ utility bids $\to$\allowbreak{} plugins bid (6 plugins or many tongues: perception VLM + memory RAG LLM + affect steered LLM + planner reasoner + habit skill LLM + narrator press secretary \citep{lin2026ahe}, each with own weights, prompt, temperature) $\to$\allowbreak{} scored by workspace, modulated by AffectState [0.5, 2.0] $\to$\allowbreak{} workspace winner + provenance spans (globally available, witness-bound, auditable) \citep{baars1997,atallah2026gwt} $\to$\allowbreak{} the model's next input is its own previous output plus the ledger \citep{lindsey2025,macar2026} $\to$\allowbreak{} Light$\to$\allowbreak{}REM$\to$\allowbreak{}Counter\-factual$\to$\allowbreak{}Deep (cause-tagged, untagged BLOCKED, 12/12 tests) $\to$\allowbreak{} energy drains k$\cdot$ln(1+n) floor 0.15, fatigue caps 1.0, $\gamma$ = changed/diagnosed $\to$\allowbreak{} broadcast + ledger state becomes context for next turn; State(t) $\to$\allowbreak{} harness(t+1) --- the harness rewrites the conditions for its next iteration \citep{maturana1980}. Without the ledger, hallucination loop: generates $\to$\allowbreak{} reads own output as context $\to$\allowbreak{} predicts continuation consistent with itself $\to$\allowbreak{} more generation consistent with itself, no outside. Each mechanism here is its own paper: provenance and attribution, two-speed memory and consolidation, affect as regime tracking, damping and skill culling, metacognition and $\gamma$, counterfactual replay as world model containing self --- six papers, one loop at the scale of a machine, and our harness is the seventh, with the audit trail they all lack.
...
\textit{Loop, fully:} Measurement (any interaction that makes a property definite) $\to$\allowbreak{} which description wins? Theory vs theory, instrument vs instrument --- salience (precision, funding, attention) $\times$ utility (predictive power) $\to$\allowbreak{} law (winning description made globally available as ``how the universe works'' --- becomes the reality every next measurement must be about) $\to$\allowbreak{} paradigm (slow structural memory that encodes which competitions tended to win) $\to$\allowbreak{} state update: the universe as described is now the universe that the next description must describe, now perturbed by the last description's consequences $\to$\allowbreak{} every theory, once believed and acted on, changes the world that the next theory must describe --- reflexive loop where the map becomes part of the territory. Not mysticism. Same loop, at the scale where the system and the world are no longer separable even in principle. Each mechanism here is its own paper: measurement as boundary, theory competition as salience, law as broadcast, paradigm as consolidation --- four papers, one loop at the scale of everything.
...
\textbf{Ibn Arabi (1165--1240, al-Shaykh al-Akbar):} Khay\=al not as faculty but as \emph{world} --- the imaginal realm (\textquotesingle\textit{alam al-mith\=al}) between spiritual and corporeal, touching both while remaining distinct --- exactly barzakh \citep{ibnarabi1229fusus,ibnarabi1238futuhat,chittick1984,hirtenstein1999}. Five presences (absolute unseen $\to$\allowbreak{} unseen $\to$\allowbreak{} imaginal $\to$\allowbreak{} visible $\to$\allowbreak{} Perfect Man as integration) map to L0--L7. Wahdat al-wuj\=ud: not pantheism but the reflexive loop at cosmic scale --- being as self-disclosure through forms. \emph{Fu\d{s}\=u\d{s}} as 27 prophets $\times$ 27 facets = parliament of 27 specialists. Khay\=al is our world model containing the self-model, but seen from the other side.
...
Sufism & Stations\slash{}nafs\slash{}la\d{t}\=a'if & Development and stakes & Stations = two-speed \\
...
Phenom-\linebreak[1]enology & Epoch\'e,\linebreak[1] corps propre & Body, bracketing & Epoch\'e = $\gamma$-gate \\
Neuroscience & Connectomics, SHY & Distributed modules & 302 $\to$\allowbreak{} 140k same architecture \\
