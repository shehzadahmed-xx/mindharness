...
mkdir -p "$OUT"
cd "$E" || exit 1
export PYTHONPATH="$E" SPRINGFISH_LLM_MODE=openai
export LLM_MODEL_NAME="Qwen3.5-0.8B-Q3_K_S"
export LLM_BASE_URL="http://127.0.0.1:18555/v1"
for S in 1 2 3 4 5 6 7 8; do
  if [ -d "$OUT/seed_$S.json" ]; then
    echo "seed $S already"
  else
    SPRINGFISH_SEED=$S $V -m springfish.cli run --case $CASE --output "$OUT/seed_$S.json" >/dev/null 2>&1
